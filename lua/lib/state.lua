...
function M.register_provider(spec)
  ...
  if M._providers[spec.id] then vim.notify(string.format("[state] Duplicate provider: '%s'", spec.id), vim.log.levels.WARN) end
  ...
  vim.api.nvim_create_autocmd(spec.events, {
    ...
    callback = function()
      ...
      if ok then
        ...
        vim.notify(string.format("[state] Provider '%s' error: %s", spec.id, value), vim.log.levels.WARN)
      end
    end,
  })
  ...
  local ok, value = pcall(spec.collect)
  if ok then M._store[spec.id] = value end
end
...
function M._update(id, value) M._store[id] = value end
...
function M.status()
  ...
  for _, key in ipairs(keys) do
    ...
    local rendered = type(value) == 'table' and vim.inspect(value):gsub('\n', ' ') or tostring(value)
  ...
  for id, _ in pairs(M._providers) do
    if snapshot[id] == nil then table.insert(uncollected, id) end
  end
