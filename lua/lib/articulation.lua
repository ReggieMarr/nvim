...
function M.register(spec)
  ...
  if not spec.desc or spec.desc == "" then
    error(string.format(
      "[articulation] Action '%s' from module '%s' must have a description",
      spec.id, spec.module
    ))
  end

  if M._actions[spec.id] then
    vim.notify(
      string.format(
        "[articulation] Duplicate action id '%s' from module '%s', overwriting",
        spec.id, spec.module
      ),
      vim.log.levels.WARN
    )
  ...
  if not ok then
    ...
    vim.notify(
      string.format(
        "[articulation] Could not create command ':%s' for action '%s'",
        cmd_name, spec.id
      ),
...
function M._bind(action_id, binding)
  local modes  = type(binding.mode) == "table" and binding.mode
    or { binding.mode or "n" }
  ...
  for _, mode in ipairs(modes) do
    ...
    if M._bindings[key] then
      ...
      if existing_id ~= action_id then
        vim.notify(
          string.format(
            "[articulation] Binding conflict: '%s' (%s) used by '%s', overwriting with '%s'",
            binding.lhs, mode, existing_id, action_id
          ),
          vim.log.levels.WARN
        )
      end
    ...
    vim.keymap.set(mode, binding.lhs, function()
      M.execute(action_id, nil, binding.buffer)
    end, {
...
function M.execute(action_id, params, buffer)
  ...
  if not action then
    vim.notify(
      string.format("[articulation] Unknown action: '%s'", action_id),
  ...
  if action.when then
    ...
    if not ok then
      vim.notify(
        string.format(
          "[articulation] Precondition error for '%s': %s",
          action_id, result
        ),
        vim.log.levels.WARN
      )
    ...
    if not result then
      vim.notify(
        string.format(
          "[articulation] Action '%s' precondition not met",
          action_id
        ),
        vim.log.levels.INFO
      )
  ...
  if not ok then
    vim.notify(
      string.format("[articulation] Action '%s' failed: %s", action_id, err),
      vim.log.levels.ERROR
    )
    return false
  end
    ...
    for _, action in ipairs(actions) do
      ...
      if action.bindings then
        for _, binding in ipairs(action.bindings) do
          local modes = type(binding.mode) == "table"
            and table.concat(binding.mode, ",")
            or (binding.mode or "n")
          table.insert(lines, string.format(
            "    [%s] %s%s",
            modes,
            binding.lhs,
            binding.buffer and (" @buf:" .. binding.buffer) or ""
          ))
        end
      end
