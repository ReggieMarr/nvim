...
local function validate_interface(impl, interface, name)
  ...
  for key, _ in pairs(interface) do
    if impl[key] == nil then table.insert(errors, string.format("  missing key: '%s'", key)) end
  end
...
function M.register(name, impl, provider)
  if M._capabilities[name] then
    vim.notify(
      string.format("[capabilities] '%s' already provided by '%s', overwriting with '%s'", name, M._capabilities[name].provider, provider),
      vim.log.levels.WARN
    )
  end
  ...
  if M.interfaces[name] then
    ...
    if not ok then
      vim.notify(string.format("[capabilities] '%s' from '%s' has interface gaps:\n%s", name, provider, table.concat(errors, '\n')), vim.log.levels.WARN)
    end
  end
...
function M.extend(name, extensions, provider)
  ...
  for key, value in pairs(extensions) do
    if M._capabilities[name].impl[key] then
      vim.notify(string.format("[capabilities] '%s' extension from '%s' overwrites '%s'", name, provider, key), vim.log.levels.DEBUG)
    end
...
function M.require(name)
  ...
  if not impl then error(string.format("[capabilities] Required capability '%s' is not registered. " .. 'Check feature flags and module load order.', name)) end
  return impl
end

---@param name string
---@return boolean
function M.has(name) return M._capabilities[name] ~= nil end
  ...
  for name, cap in pairs(M._capabilities) do
    ...
    if interface then
      ...
      for key, _ in pairs(cap.impl) do
        if interface[key] == nil then table.insert(lines, string.format('  + %s  (extension)', key)) end
      end
    else
