...
function M.update_ranges(bufnr)
  ...
  client:request('textDocument/foldingRange', params, function(err, ranges)
    if err or not ranges or not vim.api.nvim_buf_is_valid(bufnr) or tick ~= vim.b[bufnr].changedtick then return end
...
function M.update_current_fold(row, bufnr)
  ...
  for i = 1, #ranges do
    ...
    if row <= range.end_line then best_range = range end
  end
...
function M.goto_previous_fold()
  ...
  for i = #ranges, 1, -1 do
    ...
    if start_line < row then return vim.api.nvim_win_set_cursor(0, { start_line, 0 }) end
  end
end
...
function M.statuscol()
  ...
  if cursor_fold and this_range.start_line == cursor_fold.start_line then hl = 'CursorLineNr' end
