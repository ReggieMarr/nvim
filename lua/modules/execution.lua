    ...
    -- Overseer: structured task running
    {
      ...
      config = function()
        ...
        env.capabilities.register('task_runner', {
          ...
          list = function() return overseer.list_tasks() end,

          toggle = function() overseer.toggle { direction = 'bottom' } end,
        }, 'execution')

        -- Feed task state into env.state
        -- Overseer provides a subscription mechanism for task events
        overseer.on_task_create(
          function(task)
            env.state._update(
  ...
  articulation = {
    {
      ...
      config = function()
        ...
        env.capabilities.register('terminal', {
          toggle = function(opts) snacks.terminal.toggle(nil, opts) end,
          run = function(cmd, opts) snacks.terminal.open(cmd, opts) end,
        ...
        env.articulation.register_group('execution', {
          {
            ...
            handler = function() env.use('task_runner').toggle() end,
            desc = 'Toggle task list',
            bindings = { { lhs = '<leader>tt' } },
          },
          {
            id = 'execution.run_task',
            handler = function() require('overseer').run_template() end,
            desc = 'Run task',
          ...
          },
          {
            ...
            handler = function()
              ...
              if tasks[1] then require('overseer').run_action(tasks[1], 'restart') end
            end,
          ...
          },
          {
            ...
            handler = function() env.use('terminal').toggle() end,
        ...
        env.capabilities.extend('picker', {
          tasks = function(opts) env.articulation.execute('execution.find_tasks', opts) end,
        }, 'execution')
      end,
    },
  },
