...
return env.module.register({
  ...
  setup = function()
    ...
    env.state.register_provider({
      ...
      collect = function()
        ...
        local root = env.state.get("workspace.root")
          or env.state.get("workspace.cwd")
          or vim.fn.getcwd()
    ...
    env.articulation.register_group("filesystem", {
      ...
      {
        id       = "reveal_current_file",
        handler  = function()
          env.use("filesystem").reveal_file()
        end,
        desc     = "Reveal current file in tree",
        bindings = { { lhs = "<leader>xr" } },
        when     = function(state)
      ...
      },
      {
        id       = "close_tree",
        handler  = function()
          env.use("filesystem").close_tree()
        end,
        desc     = "Close file tree",
        bindings = { { lhs = "<leader>xq" } },
        when     = function(state)
          return state["tree_visible"] == true
        end,
      },
      ...
      },
      {
        ...
        bindings = { { lhs = "<leader>xf" } },
        when     = function(state)
          return state["workspace.root"] ~= nil
        end,
      },
      {
        id       = "find_directories",
        handler  = function()
          env.use("picker").directories()
        end,
        desc     = "Find directories",
        bindings = { { lhs = "<leader>xd" } },
      },
      ...
      callback = function(event)
        ...
        if client and client.config.root_dir then
          ...
          env.state._update(
            ...
            vim.fn.fnamemodify(client.config.root_dir, ":t")
          )
        end
      end,
    })
