...
function BufLinesShow.new(source_buf, source_win)
  ...
  self.lang = vim.treesitter.language.get_lang(vim.bo[source_buf].filetype)
  self.content_col = 8 -- "  NNNN: " is 8 chars
  return self
end

function BufLinesShow:_get_buf_header()
  local buf_name = vim.api.nvim_buf_get_name(self.source_buf)
  local rel = vim.fn.fnamemodify(buf_name, ':p')
  return rel ~= '' and rel or '[No File]'
end

function BufLinesShow:_apply_ts_highlights(buf_id, items_to_show, line_offset)
  if not self.lang then return end

  local ok, parser = pcall(vim.treesitter.get_parser, self.source_buf, self.lang)
  if not ok or not parser then return end
  ...
  local root = tree:root()
  local query_ok, query = pcall(vim.treesitter.query.get, self.lang, 'highlights')
  if not query_ok or not query then return end
  ...
  for i, item in ipairs(items_to_show) do
    ...
    for id, node in query:iter_captures(root, self.source_buf, src_lnum_0, src_lnum_0 + 1) do
      local capture_name = query.captures[id]
      local hl_group = '@' .. capture_name .. '.' .. self.lang
      if vim.fn.hlexists(hl_group) == 0 then hl_group = '@' .. capture_name end
...
function BufLinesShow:_render_placeholder(buf_id)
  -- TODO this should be something unselectable, maybe leverage virtual text here
  local header = self:_get_buf_header()
  local win = vim.fn.bufwinid(buf_id)
  local height = win ~= -1 and vim.api.nvim_win_get_height(win) or 20
  local placeholder = { header }
  for _ = 2, height do
    placeholder[#placeholder + 1] = ''
  end
  vim.api.nvim_buf_set_lines(buf_id, 0, -1, false, placeholder)
  vim.hl.range(buf_id, self.ns, 'DiagnosticWarn', { 0, 0 }, { 0, -1 })
end
